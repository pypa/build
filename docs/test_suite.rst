...
where the last will disable the unit tests and only run the integration ones.
Even though these tests are disabled by default, they will be run in the CI,
...
We have a fairly big environment matrix. We run tests for all supported Python
versions and implementations, and with the module being invoked by either path,
sdist install, and wheel install. Additionally, we have an environment for type
...
We have CI testing, where we the test suite accross all supported operating
